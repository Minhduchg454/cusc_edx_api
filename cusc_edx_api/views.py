...
def course_detail(request, course_id):
    ...
    image_url = (
        ...
        if callable(course.course_image_url)
        else course.course_image_url
    )

    data = {
        "course_id": str(course.id),
        "org": course.id.org,
        "course_number": course.id.course,
        "run": course.id.run,
        "display_name": course.display_name,
        "short_description": course.short_description,
        "overview": getattr(course, "overview", None)
            or getattr(course, "overview_text", None),
        ...
        "enrollment_start": (
            course.enrollment_start.isoformat()
            if course.enrollment_start else None
        ),
        "enrollment_end": (
            course.enrollment_end.isoformat()
            if course.enrollment_end else None
        ),
    }
